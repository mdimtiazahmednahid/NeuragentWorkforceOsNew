//#region node_modules/@alpinejs/sort/dist/module.esm.js
...
function _typeof(obj) {
	...
	if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") _typeof = function(obj2) {
		return typeof obj2;
	};
	else _typeof = function(obj2) {
...
function index(el, selector) {
	...
	while (el = el.previousElementSibling) if (el.nodeName.toUpperCase() !== "TEMPLATE" && el !== Sortable.clone && (!selector || matches(el, selector))) index2++;
	return index2;
}
...
var PluginManager = {
	mount: function mount(plugin) {
		for (var option2 in defaults) if (defaults.hasOwnProperty(option2) && !(option2 in plugin)) plugin[option2] = defaults[option2];
	...
	initializePlugins: function initializePlugins(sortable, el, defaults2, options) {
		...
		for (var option2 in sortable.options) {
			...
			if (typeof modified !== "undefined") sortable.options[option2] = modified;
		}
	},
...
function dispatchEvent(_ref) {
	...
	for (var option2 in allEventProperties) evt[option2] = allEventProperties[option2];
	if (rootEl2) rootEl2.dispatchEvent(evt);
...
var _prepareGroup = function _prepareGroup2(options) {
	function toFn(value, pull) {
		return function(to, from, dragEl2, evt) {
			...
			else if (typeof value === "function") return toFn(value(to, from, dragEl2, evt), pull)(to, from, dragEl2, evt);
			else {
...
Sortable.mount = function() {
	for (var _len = arguments.length, plugins2 = new Array(_len), _key = 0; _key < _len; _key++) plugins2[_key] = arguments[_key];
...
var autoScroll = throttle(function(evt, options, rootEl2, isFallback) {
	...
	if (scrollRootEl !== rootEl2) {
		...
		if (scrollEl === true) scrollEl = getParentAutoScrollElement(rootEl2, true);
	}
	var layersOut = 0;
...
Revert.prototype = {
	...
	onSpill: function onSpill(_ref3) {
		...
		if (putSortable2) putSortable2.captureAnimationState();
		var nextSibling = getChild(this.sortable.el, this.startIndex, this.options);
		if (nextSibling) this.sortable.el.insertBefore(dragEl2, nextSibling);
		else this.sortable.el.appendChild(dragEl2);
		this.sortable.animateAll();
		if (putSortable2) putSortable2.animateAll();
	},
	drop
};
...
function src_default(Alpine) {
	Alpine.directive("sort", (el, { value, modifiers, expression }, { effect, evaluate, cleanup }) => {
		...
		let sortable = initSortable(el, getConfigurationOverrides(el, modifiers, evaluate), preferences, (key, position) => {
...
function getConfigurationOverrides(el, modifiers, evaluate) {
	if (el.hasAttribute("x-sort:config")) return evaluate(el.getAttribute("x-sort:config"));
	if (el.hasAttribute("wire:sort:config")) return evaluate(el.getAttribute("wire:sort:config"));
	return {};
}
function initSortable(el, config, preferences, handle) {
	...
	return new sortable_esm_default(el, {
		...
		filter(e) {
			...
			return e.target.closest("[x-sort\\:item],[wire\\:sort\\:item]") ? false : true;
		},
		onSort(e) {
			if (e.from !== e.to) {
				if (e.to !== e.target) return;
			}
			let key = void 0;
			...
			if (key !== void 0 || key !== null) handle(key, position);
		},
		onStart() {
...
function getGroupName(el, modifiers) {
	if (el.hasAttribute("x-sort:group")) return el.getAttribute("x-sort:group");
	if (el.hasAttribute("wire:sort:group")) return el.getAttribute("wire:sort:group");
...
*/
//#endregion
export { module_default as default, src_default as sort };
